...
function HashtagBadge({tag,onClick}){
  return <span onClick={e=>{e.stopPropagation();onClick&&onClick(tag);}} style={{background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.12)",color:"rgba(255,255,255,0.7)",fontSize:11,padding:"2px 8px",borderRadius:20,cursor:onClick?"pointer":"default",fontFamily:"monospace",letterSpacing:0.5}}>#{tag}</span>;
}
...
function AlbumFeedItem({album,onOpen,onLike,liked,onTagClick}){
  ...
  const photos=album.photos||[];
  return(
    <div style={{width:"100%",height:"100%",background:"#0a0a0f",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",position:"relative",userSelect:"none"}}>
      {album.coverUrl&&<img src={album.coverUrl} alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",filter:"blur(20px) brightness(0.3)",transform:"scale(1.1)"}}/>}
      <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.5)"}}/>
      <div style={{position:"relative",zIndex:1,width:"100%",padding:"0 20px",display:"flex",flexDirection:"column",alignItems:"center",gap:12}}>
        <div style={{display:"flex",alignItems:"center",gap:8,background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.15)",borderRadius:20,padding:"5px 14px"}}>
          <span style={{color:"rgba(255,255,255,0.7)",fontSize:12,fontFamily:"monospace"}}>✦ GALERIA</span>
          <span style={{color:"rgba(255,255,255,0.4)",fontSize:11}}>{photos.length} foto{photos.length!==1?"s":""}</span>
        </div>
        <div style={{display:"flex",gap:6,width:"100%",justifyContent:"center"}}>
          {photos.slice(0,3).map((p,i)=>(
            <div key={i} style={{flex:1,maxWidth:110,aspectRatio:"3/4",borderRadius:10,overflow:"hidden",background:g.bg,border:"1px solid rgba(255,255,255,0.1)",transform:i===1?"scale(1.05)":"scale(0.95)"}}>
              {p.url&&<img src={p.url} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>}
            </div>
          ))}
        </div>
        <div style={{textAlign:"center",width:"100%"}}>
          {clean&&<p style={{color:"#fff",fontFamily:"'DM Serif Display',serif",fontSize:20,margin:"0 0 8px",fontWeight:400,textShadow:"0 2px 8px rgba(0,0,0,0.8)"}}>{clean}</p>}
          {tags.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:12,justifyContent:"center"}}>{tags.slice(0,5).map(t=><HashtagBadge key={t} tag={t} onClick={onTagClick}/>)}</div>}
          <div style={{display:"flex",gap:12,justifyContent:"center",alignItems:"center"}}>
            <button onClick={onOpen} style={{background:"linear-gradient(135deg,#7c3aed,#c084fc)",border:"none",borderRadius:20,padding:"8px 20px",color:"#fff",cursor:"pointer",fontSize:13,fontFamily:"'DM Serif Display',serif"}}>Ver galeria</button>
            <button onClick={onLike} style={{background:"none",border:"none",cursor:"pointer",color:liked?"#f472b6":"rgba(255,255,255,0.6)",fontSize:13,padding:0,display:"flex",alignItems:"center",gap:5}}>
              <span style={{fontSize:19}}>{liked?"♥":"♡"}</span> {fmtNum((album.likes||0)+(liked?1:0))}
            </button>
          </div>
        </div>
      </div>
...
// ── BUILD FEED: 5v,1a,5v,2a,5v,3a... com loop infinito ───────────────────────

function buildFeed(videos, albums) {
  const feed = [];
  let vi=0, ai=0, albumBatch=1;
  while(vi<videos.length || ai<albums.length){
    for(let i=0;i<5&&vi<videos.length;i++,vi++) feed.push({type:"video",data:videos[vi]});
    for(let i=0;i<albumBatch&&ai<albums.length;i++,ai++) feed.push({type:"album",data:albums[ai]});
    albumBatch++;
  }
  return feed;
}

// ── EXPLORE VIEW (infinite loop) ─────────────────────────────────────────────

function ExploreView({activeTag,onTagClick}){
  const [baseFeed, setBaseFeed] = useState([]);   // feed original
  const [feed, setFeed] = useState([]);            // feed com repetições
  ...
  useEffect(()=>{
    ...
    Promise.all([
      ...
      setBaseFeed(built);
      // Começa com 3 cópias embaralhadas para ter conteúdo suficiente
      setFeed([...built, ...buildFeed(shuffle(v),shuffle(a)), ...buildFeed(shuffle(v),shuffle(a))]);
      setLoading(false);
    });
  },[activeTag]);

  // Quando chegar perto do fim, adiciona mais conteúdo embaralhado
  useEffect(()=>{
    if(baseFeed.length === 0) return;
    if(current >= feed.length - 5){
      // Extrai videos e albums do baseFeed e embaralha de novo
      const videos = baseFeed.filter(x=>x.type==="video").map(x=>x.data);
      const albums = baseFeed.filter(x=>x.type==="album").map(x=>x.data);
      setFeed(f=>[...f, ...buildFeed(shuffle(videos), shuffle(albums))]);
    }
  },[current, feed.length, baseFeed]);
  ...
  // Renderiza só uma janela de itens ao redor do atual para performance
  const windowSize = 5;
      ...
      <div ref={containerRef}
        ...
        {loading?(
          ...
          <>
            ...
            {/* indicador de tipo (album=roxo, video=branco) */}
      ...
      {loading?<div style={{display:"flex",justifyContent:"center",marginTop:60}}><Spinner/></div>:(
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {showVideos&&videos.map(video=>{
            ...
            const bg=VIDEO_THUMBS[hashIdx(video.id,VIDEO_THUMBS.length)];
            return(
              <div key={video.id} style={{background:bg,borderRadius:12,overflow:"hidden",position:"relative",aspectRatio:"16/5",minHeight:80}}>
                {video.url&&<video src={video.url} muted style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",opacity:0.5}}/>}
                <div style={{position:"absolute",inset:0,background:"linear-gradient(to right, rgba(0,0,0,0.8) 0%, transparent 100%)"}}/>
                <div style={{position:"absolute",inset:0,padding:"12px 14px",display:"flex",flexDirection:"column",justifyContent:"center"}}>
                  <span style={{color:"rgba(255,255,255,0.5)",fontSize:10,fontFamily:"monospace",marginBottom:4}}>▶ VIDEO</span>
                  {clean&&<p style={{color:"#fff",fontFamily:"'DM Serif Display',serif",fontSize:15,margin:"0 0 5px",fontWeight:400}}>{clean}</p>}
                  {tags.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:4}}>{tags.map(t=><HashtagBadge key={t} tag={t} onClick={onTagClick}/>)}</div>}
                </div>
                <button onClick={()=>handleLike(video,"videos")} style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:liked[video.id]?"#f472b6":"rgba(255,255,255,0.5)",fontSize:12,padding:0,display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
                  <span style={{fontSize:20}}>{liked[video.id]?"♥":"♡"}</span>
...
function AdminPanel({adminPassword}){
  ...
  const uploadVideo=async()=>{
    ...
    setUploading(true);
    try{
      const fd=new FormData(); fd.append("file",videoForm.file); fd.append("title",videoForm.title);
      const res=await fetch(`${API}/admin/videos`,{method:"POST",headers:{"x-admin-password":adminPassword},body:fd});
      if(!res.ok) throw new Error((await res.json()).error);
      showToast("Vídeo publicado! ✓"); setVideoForm({title:"",file:null});
  ...
  const uploadAlbum=async()=>{
    ...
    setUploading(true);
    try{
      const fd=new FormData(); fd.append("title",albumForm.title);
      ...
      if(!res.ok) throw new Error((await res.json()).error);
      showToast(`Álbum publicado! ✓`); setAlbumForm({title:"",files:[]});
    }catch(e){showToast(e.message,false);}
    setUploading(false);
  };
  ...
  return(
    <div style={{height:"100%",overflowY:"auto",background:"#080810",position:"relative"}}>
      ...
      <div style={{borderBottom:"1px solid rgba(255,255,255,0.07)",padding:"20px 24px 0"}}>
        ...
        <div style={{display:"flex"}}>
          {[["videos","▶ Vídeos"],["albums","✦ Álbuns"],["library","Library"]].map(([key,label])=>(
            <button key={key} onClick={()=>setTab(key)} style={{background:"none",border:"none",borderBottom:`2px solid ${tab===key?"#c084fc":"transparent"}`,color:tab===key?"#c084fc":"rgba(255,255,255,0.4)",cursor:"pointer",padding:"8px 20px 12px",fontSize:14}}>{label}</button>
          ))}
        </div>
      </div>
      <div style={{padding:24}}>
        {tab==="videos"&&(
          <div>
            <p style={{color:"rgba(255,255,255,0.4)",fontSize:12,marginBottom:16}}>Use hashtags no título: <span style={{color:"rgba(255,255,255,0.7)",fontFamily:"monospace"}}>#tag1 #tag2</span></p>
          ...
          <div>
            ...
            {albumForm.files.length>0&&(
              <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:16}}>
                {albumForm.files.slice(0,8).map((f,i)=><div key={i} style={{width:52,height:52,borderRadius:6,overflow:"hidden",border:"1px solid rgba(255,255,255,0.1)"}}><img src={URL.createObjectURL(f)} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/></div>)}
            ...
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              ...
              <button onClick={uploadAlbum} disabled={uploading} style={{background:uploading?"rgba(8,145,178,0.4)":"linear-gradient(135deg,#0891b2,#67e8f9)",border:"none",borderRadius:8,padding:"12px 24px",color:"#fff",fontSize:15,cursor:uploading?"not-allowed":"pointer",fontFamily:"'DM Serif Display',serif",display:"flex",alignItems:"center",justifyContent:"center",gap:10}}>
                {uploading?<><Spinner size={20} color="#67e8f9"/> Enviando...</>:`Publicar Álbum (${albumForm.files.length} foto${albumForm.files.length!==1?"s":""})`}
              </button>
            </div>
          </div>
        )}
                ...
                return(
                  <div key={video.id} style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:10,padding:"12px 14px",display:"flex",alignItems:"center",gap:12}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <p style={{color:"#fff",fontSize:13,margin:"0 0 3px",fontFamily:"'DM Serif Display',serif",fontWeight:400,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{clean||"(sem título)"}</p>
                ...
                return(
                  <div key={album.id} style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:10,padding:"12px 14px",display:"flex",alignItems:"center",gap:12}}>
                    <div style={{width:34,height:34,borderRadius:6,overflow:"hidden",flexShrink:0}}>
                      {album.coverUrl?<img src={album.coverUrl} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<div style={{width:"100%",height:"100%",background:"rgba(103,232,249,0.15)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>✦</div>}
                    </div>
                    <div style={{flex:1,minWidth:0}}>
                      <p style={{color:"#fff",fontSize:13,margin:"0 0 3px",fontFamily:"'DM Serif Display',serif",fontWeight:400,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{clean||"(sem título)"}</p>
