...
const upload = multer({
  ...
  fileFilter(req, file, cb) {
    ...
    allowed.includes(file.mimetype) ? cb(null,true) : cb(new Error(`Tipo não permitido: ${file.mimetype}`));
  },
});
...
// Tags com thumb (foto de capa de um post que usa a tag)
app.get("/tags", async (req,res) => {
  try {
    const [videos, albums] = await Promise.all([db.getVideos(), db.getAlbums()]);
    const tagMap = {};
    const tagCovers = {};
    for (const item of [...videos,...albums]) {
    ...
    const tags = Object.entries(tagMap)
      .map(([tag,count])=>({ tag, count, coverUrl: tagCovers[tag]||null }))
...
app.post("/comments", async (req,res) => {
  try {
    ...
    if (!postId||!postType||!text?.trim()) return res.status(400).json({error:"postId, postType e text são obrigatórios"});
    const comment = await db.addComment({ id:uuid(), postId, postType, text:text.trim() });
...
app.post("/admin/login", (req,res) => {
  req.body.password===process.env.ADMIN_PASSWORD ? res.json({ok:true}) : res.status(401).json({error:"Senha incorreta"});
});
...
app.post("/admin/albums", requireAdmin, upload.array("files",50), async (req,res) => {
  try {
    ...
    const tags = extractTags(title);
    const album = await db.addAlbum({id:uuid(),title,tags,photos,coverUrl:photos[0]?.url||null,photoCount:photos.length,likes:0,createdAt:new Date().toISOString()});
